...
- 依赖冲突：verl-latest 要 `vllm>=0.8.5,<=0.12.0`，当前 VLM env 是 0.19.1。**完全不动 VLM env**，FIPO 跑在独立 conda env `VLM_FIPO`（[scripts/setup_fipo_env.sh](scripts/setup_fipo_env.sh) 一键安装）
...
| [src/stage3_fipo/verl_patches/future_kl_loss.py](src/stage3_fipo/verl_patches/future_kl_loss.py) | 注册 `policy_loss=future_kl` |
...
| [src/stage3_fipo/prepare_fipo_data.py](src/stage3_fipo/prepare_fipo_data.py) | SFT parquet → verl 多模态 RL parquet（含 ground_truth） |
...
| [src/stage3_fipo/run_fipo_v1.sh](src/stage3_fipo/run_fipo_v1.sh) | 单节点启动脚本（FSDP offload + vLLM rollout + GRPO + future_kl） |
| [scripts/setup_fipo_env.sh](scripts/setup_fipo_env.sh) | 一次性建独立 conda env `VLM_FIPO` |

**用户操作三步**（机器空时）：

```bash
# 一次性
bash scripts/setup_fipo_env.sh                          # 建 VLM_FIPO env，~30 min

# 一次性（每次数据更新后重跑）
conda activate VLM_FIPO
...
**v1 设计取舍**：
- **不在 reward_manager 里调 RM**：避免 actor + ref + 17GB RM 同时占显存；rule-based reward 已经覆盖五个核心信号（格式 / label / 长度 / 词表 / 语义对齐），先看效果。如果 rule reward 有顶到天花板的迹象再加 RM（reward_manager 已留 `enable_rm_score=True` 接口）
