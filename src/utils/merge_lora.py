"""
LoRA merge utility: fold LoRA adapters back into the base model weights.

Useful when downstream tooling (rollout engines, deployment pipelines) does
not support attached PEFT adapters and needs a fully-materialised model.

Usage:
    python -m src.utils.merge_lora \
        --base_model models/pretrained/Qwen3-VL-8B-Instruct \
        --lora_path  models/prm_ckpt/epoch-3 \
        --out_dir    models/prm_merged
"""
